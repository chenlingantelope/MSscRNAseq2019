...
count = count[,celllabels=='CD4']
batchid = batchid[celllabels=='CD4']
state = state[celllabels=='CD4']
tissue = tissue[celllabels=='CD4']
celllabels = read.csv('RFiles/CD4labels.csv',header=F)[,1]
...
qlf <- glmQLFTest(fit)
write.csv(qlf$table, file=paste('EdgeR/CD4.batch_corrected.',i,'.edgeR.csv',sep=''))
